...
def regenerate_context():
    ...
    if vision_meta.exists():
        full_context += "\n### 👁️ VISUAL STANDARDS\n" + vision_meta.read_text(encoding="utf-8")
    ...
    if not biz_files:
        ...
        if opcion == "2":
            ...
            if 0 <= idx < len(biz_files):
                biz_content = (biz_templates_path / f"{biz_files[idx]}.md").read_text(encoding="utf-8")
        
        if not biz_content and opcion != "2" and not (opcion.isdigit() and int(opcion) > 2):
            rules = input("Reglas manuales: ")
            if rules: biz_content = f"# BUSINESS RULES\n{rules}"

    if biz_content: (standards_dest / "business.md").write_text(biz_content, encoding="utf-8")
...
def manage_skills(args):
    ...
    if action == "list":
        ...
        print(" Usa 'rapid skill add <nombre>' para instalar miles de skills de la comunidad.")
        print(" Ej: rapid skill add vercel-labs/agent-skills")
        return

    # MODO 2: INSTALAR REMOTO (VERCEL CLI)
    if action == "add":
        if not skill_name: print_error("Especifica el nombre (ej. vercel-labs/agent-skills).")
    ...
    if action == "install":
        if not skill_name: print_error("Especifica el nombre del template local.")
        
        src_skill = TEMPLATES_DIR / "skills" / skill_name
        if not src_skill.exists():
            print_error(f"Template '{skill_name}' no existe. Usa 'add' para buscar en remoto.")

        config = load_project_config()
        tools = config.get("tools", ["cursor", "claude", "antigravity"])

        targets = []
        if "cursor" in tools: targets.append(CURRENT_DIR / ".cursor" / "skills")
        if "claude" in tools: targets.append(CURRENT_DIR / ".claude" / "skills")
        if "antigravity" in tools: targets.append(CURRENT_DIR / ".agent" / "skills")
        ...
        for target_root in targets:
            ...
            except Exception as e: print_warning(f"Error: {e}")
...
def generate_mcp_config(args):
    ...
    if not topo_file.exists(): print_error("Ejecuta 'init' primero.")
    ...
    if "postgres" in topo_content and "supabase" not in topo_content:
        ...
        if tpl_path.exists():
            try: mcp_config["mcpServers"].update(json.loads(tpl_path.read_text()))
            except: pass
...
def main():
    ...
    deploy = subparsers.add_parser("deploy"); deploy.add_argument("target", nargs="?")
    vision = subparsers.add_parser("vision"); vision.add_argument("path")
    subparsers.add_parser("mcp")
    refine = subparsers.add_parser("refine"); refine.add_argument("file")
    subparsers.add_parser("guide")

    args = parser.parse_args()

    if args.command == "init": init_project(args)
    elif args.command == "skill": manage_skills(args)
