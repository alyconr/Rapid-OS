...
def generate_mcp_config(args):
    ...
    if "postgres" in topo_content and "supabase" not in topo_content:
        ...
        if tpl_path.exists():
            ...
            except Exception as e: print_warning(f"Error leyendo template postgres: {e}")
    ...
    if "supabase" in topo_content:
        ...
        if tpl_path.exists():
            ...
            try:
                data = json.loads(tpl_path.read_text(encoding="utf-8"))
                mcp_config["mcpServers"].update(data)
            except Exception as e: print_warning(f"Error leyendo template supabase: {e}")
    ...
    print("👉 IMPORTANTE: Revisa el archivo generado y actualiza las credenciales (USER/PASS/URL).")
    print("👉 Luego copia este contenido a la configuración de tu Agente (Cursor/Claude).")
...
def show_guide():
    topics = {"1": "Rapid OS Basics", "2": "Daily Workflow", "3": "Scope Wizard", "4": "Vision Command", "5": "MCP Tools (New)"}
    clear_screen()
...
def main():
    ...
    elif args.command == "mcp": generate_mcp_config(args) # <--- CONECTADO
